...
class AdminRepository:
    ...
    def get_all(self) -> list[AdminDTO]:
        sql = "SELECT id, name, email, password, created_at FROM admins"
        result = self.db.fetch_all(sql)
        return [
            AdminDTO(
                id=row[0], name=row[1], email=row[2], password=row[3], created_at=row[4]
            )
            for row in result
        ]

    def get_by_id(self, id: int) -> AdminDTO:
        sql = "SELECT id, name, email, password, created_at FROM admins WHERE id = %s"
    ...
    def get_by_email(self, email: str) -> AdminDTO:
        sql = (
            "SELECT id, name, email, password, created_at FROM admins WHERE email = %s"
        )
